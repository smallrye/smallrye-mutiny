...
public class UniOnFailure<T, TT extends Throwable> {
    ...
    public UniOnFailure(Uni<T> upstream, Class<TT> typeOfFailure, Predicate<? super Throwable> predicate) {
     ...
     */
    @CheckReturnValue
    public Uni<T> invoke(Consumer<TT> callback) {
        Consumer<TT> actual = Infrastructure.decorate(nonNull(callback, "callback"));
     ...
     */
    @CheckReturnValue
    public Uni<T> call(Function<TT, Uni<?>> action) {
        Function<TT, Uni<?>> actual = Infrastructure.decorate(nonNull(action, "action"));
     ...
     */
    @CheckReturnValue
    public Uni<T> transform(Function<TT, ? extends Throwable> mapper) {
        Function<TT, ? extends Throwable> actual = Infrastructure.decorate(nonNull(mapper, "mapper"));
     ...
     */
    @CheckReturnValue
    public Uni<T> recoverWithItem(Function<TT, ? extends T> function) {
        Function<TT, ? extends T> actual = Infrastructure.decorate(nonNull(function, "function"));
     ...
     */
    @CheckReturnValue
    public Uni<T> recoverWithUni(Function<TT, Uni<? extends T>> function) {
        Function<TT, Uni<? extends T>> actual = Infrastructure.decorate(nonNull(function, "function"));
        return Infrastructure.onUniCreation(
