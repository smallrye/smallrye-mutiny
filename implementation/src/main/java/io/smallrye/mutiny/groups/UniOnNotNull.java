...
public class UniOnNotNull<T> {
    ...
    /**
     * Transforms the received item <em>asynchronously</em>, forwarding the events emitted by another {@link Uni} produced by
    ...
    public <R> Uni<R> applyUni(Function<? super T, ? extends Uni<? extends R>> mapper) {
        return upstream.onItem().applyUni(item -> {
            if (item != null) {
                return mapper.apply(item);
            } else {
                return Uni.createFrom().nullItem();
    ...
    /**
     * When this {@code Uni} produces its item (not {@code null}), call the given {@code mapper} to produce
     * a {@link Publisher}. Continue the pipeline with this publisher (as a {@link Multi}). The events generated by this
    ...
    public <R> Multi<R> applyMulti(Function<? super T, ? extends Publisher<? extends R>> mapper) {
        return upstream.onItem().applyMulti(item -> {
            if (item != null) {
                return mapper.apply(item);
            } else {
    ...
    /**
     * Transforms the received item <em>asynchronously</em>, forwarding the events emitted by another {@link CompletionStage}
     * produced by the given {@code mapper}.
     * <p>
     * The mapper is called with the item event of the current {@link Uni} and produces an {@link CompletionStage},
     * possibly using another type of item ({@code R}). The events fired by produced {@link CompletionStage} are
    ...
    public <R> Uni<R> applyCompletionStage(Function<? super T, ? extends CompletionStage<? extends R>> mapper) {
        return upstream
                .onItem().applyCompletionStage(item -> {
                    if (item != null) {
                        return mapper.apply(item);
                    } else {
    ...
    /**
     * Transforms the received item <em>asynchronously</em>, forwarding the events emitted an {@link UniEmitter} consumes by
     * the given consumer.
     * <p>
     ...
     *         in an asynchronous manner.
     * @deprecated Use {@link #applyUni(BiConsumer)} instead.
     */
    @Deprecated
    ...
    public <R> Uni<R> applyUni(BiConsumer<? super T, UniEmitter<? super R>> consumer) {
        return upstream.onItem().applyUni((item, emitter) -> {
            if (item != null) {
