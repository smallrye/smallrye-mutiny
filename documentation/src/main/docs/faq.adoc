...
You can also transform items asynchronously using the `applyUni` / `applyCompletionStage` operators:
...
When dealing with multis, you can use:

* `applyUniAndConcatenate` - run the async operation and concatenate the produced items preserving the order from upstream
* `applyUniAndMerge` - run the async operation and merge the produced items as soon as they are produced.
* `applyUni` - return an object letting you configure the _flattening_ process (merge or concatenate)

If you need to generate a sequence of items use `applyMulti` / `applyMultiAndMerge` / `applyMultiAndConcatenate`:
...
To chain asynchronous calls, use `onItem().applyUni` or `onItem().applyCompletionStage`:
...
List<String> list = multi
    ...
    // configure the flattening process here using .merge() or .concatenate
----
...
`applyUniAndConcatenate()` preserves the ordering. So, the output will always be `A`, `B`, `C`.
`applyUniAndMerge()` emits the resulting items as soon as they are available, and so may interleave the items.
...
`applyUni` returns an object to configure how the produced items are going to be propagated downstream (flattening).
