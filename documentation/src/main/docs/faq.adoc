...
Mutiny also provides the `invokeUni` method for executing an action returning a `Uni`.
This is useful to execute an asynchronous action without modifying incoming item:

[source,java,indent=0]
----
include::../../../src/test/java/snippets/EventsTest.java[tags=invoke-uni]
----
...
|`uni.eventually(() -> uni2)`
|`uni.onItemOrFailure().invokeUni((ignoredItem, ignoredException) -> uni2)`
|===
...
The `onCancellation().invokeUni(...)` method is called when the `Uni` subscription is cancelled.
