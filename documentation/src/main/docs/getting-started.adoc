...
Here, this `Multi` is emitting to _item_ events `hello` and `world`.
...
Then, when the upstream send the `completion` event (no more items to emit), we produce a last item: `!` and send it downstream.
