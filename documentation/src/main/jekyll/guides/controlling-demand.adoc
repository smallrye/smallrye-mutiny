...
[source,java,indent=0]
----
include::{include_dir}/ControlDemandTest.java[tags=capConstant]
----
...
[source,java,indent=0]
----
include::{include_dir}/ControlDemandTest.java[tags=capFunction]
----

Here we have a function that requests 75% of the downstream requests.

Note that the function must return a value `n` that satisfies `(0 < n &leq; requested)` where `requested` is the downstream demand.
